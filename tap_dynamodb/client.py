...
class DynamoDBStream(Stream):
    ...
    def get_records(self, context: Optional[dict]) -> Iterable[dict]:
        ...
        if self.replication_method == "FULL_TABLE":
            ...
            if not singer.get_bookmark(
                self.tap_state, self.name, "initial_full_table_complete"
            ):
                self.logger.info(
                    f"Must complete full table sync before replicating "
            ...
            self.logger.info(
                f"Unknown replication method: {self.replication_method} for stream: {self.name}"
            )

    def full_table_get_records(self):
        results = full_table.scan_table(
            self.name, self.orig_projection, None, self.config, False
        )
    ...
    def log_based_get_records(self):
        ...
        try:
            ...
            self.logger.error(
                "Streams are not enabled for this table. Please use FULL_TABLE replication"
            )
            raise RuntimeError(
                "Streams are not enabled for this table. Please use FULL_TABLE replication"
            )
        ...
        # fully synced
        seq_number_bookmarks = singer.get_bookmark(
            state, table_name, "shard_seq_numbers"
        )
        ...
        finished_shard_bookmarks = singer.get_bookmark(
            state, table_name, "finished_shards"
        )
        ...
        for shard in log_based.get_shards(streams_client, stream_arn):
            ...
            if shard["ShardId"] not in finished_shard_bookmarks:
                records = self.process_shard(
                    shard,
                    seq_number_bookmarks,
                    streams_client,
                    stream_arn,
                    self.orig_projection,
                    deserializer,
                    table_name,
                    state,
                )
            ...
            if seq_number_bookmarks.get(shard["ShardId"]):
                ...
                state = singer.write_bookmark(
                    state, table_name, "shard_seq_numbers", seq_number_bookmarks
                )
    ...
    def process_shard(
        ...
        for record in log_based.get_shard_records(
            streams_client, stream_arn, shard, seq_number
        ):
            ...
            seq_number_bookmarks[shard["ShardId"]] = record["dynamodb"][
                "SequenceNumber"
            ]
