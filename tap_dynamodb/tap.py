...
class TapDynamoDB(Tap):
    ...
    def discover_streams(self) -> List[Stream]:
        ...
        try:
            ...
            raise Exception(
                "Authorization to AWS failed. Please ensure the role and "
                "policy are configured correctly on your AWS account."
            )

        config_table_list = self.config.get("tables_to_discover")
        table_list = (
            config_table_list if config_table_list else response.get("TableNames")
        )

        streams = [
            x
            for x in (self.discover_table_schema(client, table) for table in table_list)
            if x is not None
        ]
